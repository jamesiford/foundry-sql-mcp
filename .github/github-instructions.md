...
- On `demo/public-evaluation`, follow `docs/demo-roadmap.md` and implement SQL MCP Server only over disposable Entra-only Azure SQL Database. Do not add Foundry IQ, Search knowledge sources, knowledge bases, embedding models, or an IQ agent. Document SQL MI and on-prem SQL Server as customer variants. Keep public demo infrastructure isolated from `main` and preserve the cleanup path.
